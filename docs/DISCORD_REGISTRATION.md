...
Docmostアカウントを作成できます。管理者は複数の「DiscordサーバーID＋ロール条件」
を登録でき、いずれかの条件に一致した人だけが登録できます。1つの条件には複数の
ロールIDを設定し、ORまたはANDで判定できます。
...
表示名、DiscordサーバーID、1～10個のDiscordロールIDを入力して許可設定を
追加します。ロールIDは入力するたびにEnterキーを押してください。
表示名は用途を限定しない管理用の名前です。会社、学校、チーム、コミュニティなど、
利用する組織に合わせて自由に設定できます。

DiscordのIDは、Discordのユーザー設定で開発者モードを有効にした後、対象の
サーバーまたはロールを右クリックして「IDをコピー」で取得できます。

- 「いずれか（OR）」は、指定したロールを1つ以上持つ人を許可します。
- 「すべて（AND）」は、指定したロールをすべて持つ人だけを許可します。
- 複数サーバーを許可する場合は、サーバーごとに許可設定を追加します。
- 複数の許可設定同士はORです。どれか1つの設定に一致すれば登録できます。
